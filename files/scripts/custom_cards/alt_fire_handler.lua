...
---@param full_auto boolean should the card fire while holding down?
function AltFireHandler(card, projectile, cooldown_frames, vel_min, vel_max, mana_cost, full_auto)
    ...
    if now >= cooldown then
        if (function ()
            if full_auto then return ComponentGetValue2(controlscomp, "mButtonDownRightClick")
            else return ComponentGetValue2(controlscomp, "mButtonFrameRightClick") == now end
        end)() then
