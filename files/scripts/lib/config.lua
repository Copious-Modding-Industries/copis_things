print( "[copi's things] config file called - reduce these calls as much as possible" );
                dofile_once( "mods/copis_things/files/scripts/lib/content_types.lua" );
...
local get_content_flag = function( content_id, content_table )
    ...
    if content ~= nil then
        local content_flag = string.lower("copis_things_"..find_content_type( content.type ).id.."_"..content.key);
        return content_flag;
    end
end
...
local register_content = function( content_table, content_type, key, display_name, description, options, enabled_by_default, deprecated, init_function, development_mode_only, tags, sprite, author, local_content )
    ...
    local content = {
        ...
        toggle = function( force )
            ...
            if enable then
                ModSettingSet( "copis_things_noita."..settings_key, true );
            else
                ModSettingSet( "copis_things_noita."..settings_key, false );
            end
        end,
        options = options or {},
...
-- Options Menu Start
    ...
    local OPTIONS = {
        register_option_tab_localized( "copis_things_options", 2,
            register_option_group_localized( "copis_things_mod_core", nil,
                ...
                register_boolean_option_localized( FLAGS.ShowSpellBorders, false, false, CONTENT_ACTIVATION_TYPE.Immediate, nil, nil, true ),
                register_range_option_localized( FLAGS.SpellSortMethod, 0, 0, 2, 0,
                function( value ) return math.floor( value + 0.5 ); end,
                function( value ) return ({"Localized Alphabetical","ID Alphabetical","Internal Order"})[value+1] end,
                CONTENT_ACTIVATION_TYPE.Immediate, nil, nil, true )
            ),
            register_option_group_localized( "copis_things_hero_mode", nil,
            ...
            register_option_group_localized( "copis_things_champion_enemies", nil,
            ...
            register_option_group_localized( "copis_things_random_start", nil,
            ...
            register_option_group_localized( "copis_things_gold", nil,
            ...
            register_option_group_localized( "copis_things_wands", nil,
            ...
            register_option_group_localized( "copis_things_loadouts", nil,
            ...
            register_option_group_localized( "copis_things_seeded_runs", nil,
                ...
                register_button_option_localized( MISC.SeededRuns.CopySeedButtonFlag, function( left_click, right_click )
                    if left_click == true then
                        ...
                        GlobalsSetValue( "copis_things_force_settings_refresh", "1" );
                    end
                end )
            ),
            register_option_group_localized( "copis_things_perk_options", nil,
            ...
            register_option_group_localized( "copis_things_health_damage", nil,
            ...
            register_option_group_localized( "copis_things_less_particles", nil,
            ...
            register_option_group_localized( "copis_things_ui", nil,
            ...
            register_option_group_localized( "copis_things_target_dummy", nil,
                ...
                register_button_option_localized( "copis_things_spawn_target_dummy", function( left_click, right_click )
                    if left_click == true then
                        local player = EntityGetWithTag( "player_unit")[1];
                        if player then
                            local x,y = EntityGetTransform( player );
                            EntityLoad( "mods/copis_things_noita/files/copis_things/misc/dummy_target.xml", x, y - 10 );
                        end
                    end
                end ),
                register_button_option_localized( "copis_things_spawn_boss_target_dummy", function( left_click, right_click )
                    if left_click == true then
                        local player = EntityGetWithTag( "player_unit")[1];
                        if player then
                            local x,y = EntityGetTransform( player );
                            EntityLoad( "mods/copis_things_noita/files/copis_things/misc/dummy_target_final.xml", x, y - 10 );
                        end
                    end
                end )
            ),
            register_option_group_localized( "copis_things_misc", nil,
...
local cache_content = function()
    ...
    for _,content_type in pairs( content_types ) do
        ...
        print( "[copi's things] caching "..#( content_table or {} ).." "..content_type.short_name );
    ...
    print( "[copi's things] content cached" );
end
...
local parse_content = function( first_parse, force_cached, content_table )
    ...
    if load_cached == nil then
        load_cached = has_content_been_cached or GameHasFlagRun( "copis_things_content_cached" );
    end
    ...
    for _,content_type_data in pairs( content_types ) do
        ...
        if content_type_table then
            print( "[copi's things] registering "..#content_type_table.." "..content_type_data.short_name.. ( ( load_cached and " cached" ) or "") );
        ...
        print( "[copi's things] no cache or content table found for "..index );
        end
    end
end
...
local disable_content = function( content_table )
    print( "[copi's things] disable content" );
    if content_table == nil then content_table = CONTENT; end
    ...
    for _,content_type in pairs( content_types ) do
        if #(disable_strings[ content_type.id ] or "") > 0 then
            local disable_filepath = "mods/copis_things_noita/files/copis_things/scratch/disable_"..content_type.id..".lua";
...
COPI_CONFIG.cache_content();
COPI_CONFIG.parse_content( true );