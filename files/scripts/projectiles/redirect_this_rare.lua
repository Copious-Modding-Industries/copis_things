...
if shooter > 0 then
	...
	if ComponentGetValue2( controls, "mButtonFrameFire" ) == GameGetFrameNum() or (lifetime > math.max(3, math.min(30,proj_lifetime/3)) and ComponentGetValue2( controls, "mButtonDownFire" )) then
		local magnitude = ( vel_x * vel_x + vel_y * vel_y )^0.5
		ComponentSetValue2(velcomp, "mVelocity", vel_x_new*magnitude, vel_y_new*magnitude)
		SetValueInteger("redirect_this_lifetime", 0)
	end
