...
for i=1, #hittables do
    ...
    if cdc ~= nil then
        ...
        local body_x, body_y = EntityGetTransform(hittables[i])
        local angle = math.atan2(body_y - y, body_x - x)
        local len = math.sqrt(vel_x^2 + vel_y^2)
        vel_x = (math.cos(angle) * len) + vel_x
        vel_y = (math.sin(angle) * len) + vel_y
...
for i=1, #projectiles do
    if projectiles[i] ~= entity_id then
        ...
        if vcomp ~= nil then
            ...
            local body_x, body_y = EntityGetTransform(projectiles[i])
            local angle = math.atan2(body_y - y, body_x - x)
            local len = math.sqrt(vel_x^2 + vel_y^2)
            vel_x = (math.cos(angle) * len) + vel_x
            vel_y = (math.sin(angle) * len) + vel_y
...
PhysicsApplyForceOnArea(
    function(entity, body_mass, body_x, body_y, body_vel_x, body_vel_y, body_vel_angular)
            ...
            local angle = math.atan2(body_y - y, body_x - x)
            local len = math.sqrt(body_vel_x^2 + body_vel_y^2)
            local vel_x = (math.cos(angle * -1) * len) + body_vel_x
            local vel_y = (math.sin(angle * -1) * len) + body_vel_y
