...
for _,id in ipairs(entities) do	
    if not EntityHasTag(id,"black_hole") then
...
-- attract entities
local char_entities = EntityGetInRadiusWithTag(x, y, distance_full, "hittable")
for _,id in ipairs(char_entities) do
    if id ~= who_shot then
        local physicscomp = EntityGetFirstComponent(id, "PhysicsBody2Component") or EntityGetFirstComponent( id, "PhysicsBodyComponent")
        if physicscomp == nil then
            local px, py = EntityGetTransform( id )

            local charcomp = EntityGetFirstComponent( id, "CharacterDataComponent" )
            if ( charcomp ~= nil ) then
                local fx, fy = calculate_force_at(px, py)
                edit_component( id, "CharacterDataComponent", function(comp,vars)
                    local vel_x,vel_y = ComponentGetValue2( comp, "mVelocity")
