local to_insert =
{
	...
	--  Spell Jam
	{
		...
		func = function(entity_perk_item, entity_who_picked, item_name)
			...
			local valid = {}
			for i=1, #actions do
				if actions[i].spawn_requires_flag then
			...
			SetRandomSeed(GameGetFrameNum(), entity_perk_item)

			-- Kill inventory spells and drop new ones
			if #spells > 0 then
				local x, y = EntityGetTransform(entity_who_picked)
				for index, spell in ipairs(spells) do
					local card = CreateItemActionEntity(valid[Random(1, #valid)], y + math.random(-10001, 10000), 6, index), x, y)
